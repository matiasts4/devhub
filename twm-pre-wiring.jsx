  ...
  if (window.__DEVHUB_BUILD_MARKERS__.workspacesManager !== WORKSPACES_MANAGER_BUILD_MARKER) {
    ...
    try {
      fetch('/api/terminal/log', {
        ...
        body: JSON.stringify({
          tag: 'BUILD',
          msg: `TerminalWorkspacesManager.jsx loaded ÔÇö marker=${WORKSPACES_MANAGER_BUILD_MARKER}`,
        }),
      }).catch(() => {});
    } catch {
      // never crash module load ÔÇö diagnostic only
    }
  }
}
  ...
  if (runs.length > 0) {
    ...
    if (launchId && launchRuns?.length) {
      ...
      return {
        launchId,
        title:
          latestRun?.taskTitle?.split(' ┬À ')?.[0] || latestRun?.taskTitle || 'Active swarm launch',
...
function shortenSemanticLabel(value, maxLength = 40) {
  ...
  return `${raw.slice(0, Math.max(0, maxLength - 1)).trimEnd()}ÔÇª`;
}
...
function derivePanelCommandMetadata(initialCommand) {
  ...
  if (command.toLowerCase().includes('opencode')) {
    ...
    const fullText = secondary ? `OpenCode ┬À ${secondary}` : 'OpenCode';
    return {
      source: 'command',
      primary: 'OpenCode',
      secondary,
  ...
  if (quietCommand && quietCommand !== 'Ejecucion iniciada desde terminal') {
    return {
      ...
      fullText: `Terminal ┬À ${quietCommand}`,
    };
  }
...
function derivePanelSemanticMetadata(panel, agentRun) {
  ...
  if (!agentRun) {
    ...
    return {
      ...
      fullText: `${panelSwarmRole.label} ┬À ${commandMetadata.fullText || commandMetadata.primary || 'Terminal'}`,
  ...
  const secondary = swarmRole
    ? `${agentLabel}${promptSummary ? ` ┬À ${promptSummary}` : ''}`
  ...
  if (swarmRole) {
    return {
      ...
      secondary,
      fullText: `${swarmRole.label} ┬À ${secondary || agentLabel}`,
      swarmRole,
    };
  }
  ...
  return {
    ...
    primary: agentLabel,
    secondary,
    fullText: secondary ? `${agentLabel} ┬À ${secondary}` : agentLabel,
  };
}
...
export function renderWorkspacePanel(
  ...
  // Shared-surface singleton only when pizarra owns projection ÔÇö workspace docks mount
  // TerminalTTY directly to avoid hiddenÔåÆportal remount (double xterm / double echo).
  ...
  return (
    ...
    >
      ...
      >
        {/* Agent info bar ÔÇö kept above the native terminal surface so VTE cannot cover it. */}
        ...
        />
        {/* Panel controls ÔÇö top-right, outside the native terminal body. */}
        <div
        ...
        {shouldShowSwarmStandbyOverlay(panel, swarmDelegatedRoleKeys) ? (
          ...
          >
            <div className="rounded-md border border-white/10 bg-[rgba(8,12,24,0.88)] px-3 py-2 text-center text-[11px] text-[var(--text-muted)] shadow-[0_8px_24px_rgba(0,0,0,0.35)]">
              ...
              <span className="mt-0.5 block">Esperando delegaci├│n desde ZED</span>
            </div>
          </div>
        ) : null}
      </div>
...
export default function TerminalWorkspacesManager({ cwd, isVisible, projectId }) {
  ...
  // Idempotent ÔÇö re-running on a panel that already has a displayName is a
  ...
  const handleTerminalSwarmLaunch = useCallback(async () => {
    ...
    try {
      ...
      if (runtimeRequests.length === 0) {
        ...
        throw new Error(
          failedDetail
            ? `El swarm no se lanz├│: no se pudo inicializar ning├║n agente. ${failedDetail}`
            : 'El swarm no se lanz├│: no se pudo inicializar ning├║n agente.'
        );
      }
  ...
  const updateRightDockState = useCallback((nextValue) => {
    setRightDockState((prev) => {
      ...
      // ponytail: sanitize always allocates ÔÇö skip re-render when dock is unchanged
  ...
  // Operator action cards ÔÇö consumed from OperatorActionsDispatchContext (provider lives in App.js)
  ...
  const createWorkspaceWithTerminalCount = useCallback(
    (setup = {}) => {
      ...
      if (newPanelIds.length > 0) {
        ...
        // Workspace docks mount TerminalTTY directly ÔÇö the burst only adds redundant
  ...
  const removeWorkspace = async (e, idToRemove) => {
    ...
    try {
      ...
      // Ignore localStorage failures ÔÇö cleanup is best-effort
    }
    ...
    if (typeof window !== 'undefined' && legacySurvivorPanelIds.length > 0) {
      // Closing the ACTIVE workspace lands the user on another workspace ÔÇö that
      ...
      // ÔåÆ syncTerminalViewportOnWorkspaceShow + scheduleBoundedForceRepaint retry).
      // Without this dispatch xterm destination panels relied solely on the
      // layout-show useLayoutEffect's falseÔåÆtrue repaint, which races the async GPU
      ...
      // terminal-engine-v2 panels skip survivor recovery ÔÇö they keep the PTY alive
  ...
  useEffect(() => {
    // Poll even when TWM is not the focused view ÔÇö lazy worker provision must not
  ...
  const handleClosePanel = useCallback(
    async (panelIdToClose = null) => {
      ...
      const nextColumnsSnapshot = activeWorkspace.columns
        ...
        .filter((col) => col.panels.length > 0); // Eliminar columnas vac├¡as
  ...
  // ÔöÇÔöÇÔöÇ Shared Live Surface Registry Hook & Interceptors ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
  ...
  const registryAddSurface = useCallback(
    (surface) => {
      ...
      if (surface.type === 'terminal' && !surface.panelId) {
        ...
        // Do NOT touch browserWindowState 'open' ÔÇö that exclusively drives the
  ...
  useEffect(() => {
    ...
    // 2. Browser surface for the workspace ÔÇö only include if the ws "browser is open"
  ...
  useEffect(() => {
    ...
    const handleRunAgent = async (e) => {
      ...
      // `devhub:run-agent-accepted` ack ÔÇö fire it after a successful split so
      ...
      if (taskId && createdPanelId) {
        ...
        try {
          ...
          // window/CustomEvent may be undefined in tests ÔÇö best-effort ack.
        }
      }
    };
  ...
  // broadcasts the session ID via WebSocket ÔåÆ TerminalTTY emits a DOM event ÔåÆ here we
    ...
    const handleManualReviveRequested = (e) => {
      ...
      if (sessionKind === 'opencode') {
        ...
        if (!opencodeSessionId) {
          setReopenActionError(
            'No se encontr├│ un id de sesi├│n OpenCode guardado. Abr├¡ una sesi├│n nueva o us├í pol├¡tica autom├ítica.'
          );
          return;
        }
      ...
      // Swarm: tmux reattach happens in ttyServer on connect ÔÇö no launch wrapper replay.
      ...
      // Shell gen├®rico: reconectar en cwd (sin opencode --session).
      clearSuspended();
      ...
      <SharedSurfacesProvider
        onSurfaceDestroy={(surfaceId) => {
          ...
          try {
            ...
            // ignore ÔÇö registry may already be in the right state
          }
        }}
      >
              ...
              <div className="flex items-center gap-0.5 shrink-0">
                ...
                {activeSwarmLaunchSummary?.launchId ? (
                  <span
                    ...
                    title={`${activeSwarmLaunchSummary.title} ┬À ${activeSwarmLaunchSummary.count} paneles ┬À cerr├í el workspace para finalizar`}
                  >
                    {activeSwarmLaunchSummary.title} ┬À {activeSwarmLaunchSummary.count}
                  </span>
                ) : null}
                ...
                <button
                  ...
                  title="Configuraci├│n de restauraci├│n de terminales"
                  aria-label="Configuraci├│n de restauraci├│n de terminales"
                      ...
                      resumableSessions.map((session) => (
                        ...
                        >
                          ...
                          <div className="flex items-center gap-2 w-full">
                            ...
                            <span className="text-[10px] text-gray-600">┬À</span>
                            <span className="text-[10px] text-gray-500 truncate">
                              {session.agentId}
                            </span>
                          </div>
                        ...
                        >
                          ...
                          >
                            ...
                            >
                              ...
                              {/* Terminal bodies ÔÇö PanelGroup stays mounted in focus mode (CSS only). */}
                              <div
                                  ...
                                  >
                                    <div className="flex flex-col items-center gap-2 max-w-sm">
                                      ...
                                      <p className="text-xs text-[var(--text-muted)]">
                                        Cre├í una nueva shell para seguir trabajando sin cerrar el
                                        workspace.
                                      </p>
                                    </div>
                                    <button
                                                            ...
                                                            >
                                                              ...
                                                              >
                                                                {renderWorkspacePanelSlot(panel, {
                                                                  ...
                                                                  // isVisibleInLayout toggles falseÔåÆtrue on the destination
