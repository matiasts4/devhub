# Spec: board-element-placement

> **Source of truth**: promoted from `openspec/changes/pizarra-ux-overhaul/specs/board-element-placement/spec.md` on 2026-06-01 (archive of pizarra-ux-overhaul Phase 1).
...
> **Stem rationale**: Owns the policy that decides where a new element lands on creation; deliberately separated from the existing `pizarra-canvas` (shape model + selection) and `canvas-terminal` (xterm + fit + zoom) specs.
