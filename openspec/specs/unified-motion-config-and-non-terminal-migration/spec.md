# Specification: unified-motion-config-and-non-terminal-migration

## Purpose

Define the system-wide motion-mode preference and the migration of non-terminal UI surfaces to preset-based, mode-aware motion in the DevHub web app.
...
- `motion-lab-showcase`: The Motion demos requirement was updated so the showcase initializes its mode from the global `MotionProvider` while preserving the local mode toggle. See `openspec/specs/motion-lab-showcase/spec.md`.
- `terminal-pizarra-motion-coordination`: Terminal and pizarra motion coordination remains deferred to Phase B. See `openspec/changes/unified-motion-rollout/specs/terminal-pizarra-motion-coordination/spec.md`.
