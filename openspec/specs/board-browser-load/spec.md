# Spec: board-browser-load

> **Source of truth**: promoted from `openspec/changes/pizarra-ux-overhaul/specs/board-browser-load/spec.md` on 2026-06-01 (archive of pizarra-ux-overhaul Phase 1).
...
> **Stem rationale**: Distinct from the existing `openspec/specs/browser-preview-lifecycle/spec.md` (which covers selector activation / cross-origin support classification) and `openspec/specs/browser-preview-responsiveness/spec.md` (which covers bounded polling and diagnostics). This spec owns the _initial mount and runtime mode_ contract for pizarra-mounted browser shapes.
