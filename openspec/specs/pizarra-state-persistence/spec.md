# Spec: pizarra-state-persistence (alignment)

> **Source of truth**: promoted from `openspec/changes/pizarra-ux-overhaul/specs/pizarra-state-persistence/spec.md` on 2026-06-01 (archive of pizarra-ux-overhaul Phase 1).
> **Status**: active alignment note. Owned by DevHub pizarra team.
> **Origin**: Pizarra UX Overhaul (Phase 1) — alignment note for the in-flight `pizarra-state-persistence` change.
> **Note**: this spec does NOT ship a persistence implementation. It documents the source-of-truth state shape so the in-flight `pizarra-state-persistence` change can adopt the array-based reducer without a second migration. The actual localStorage / persistence implementation lives in the in-flight `pizarra-state-persistence` change.
> **Move coverage**: this is the Move 6 alignment note for `pizarra-state-persistence`.
