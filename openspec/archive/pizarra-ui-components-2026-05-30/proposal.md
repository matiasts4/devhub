...
- [x] Pizarra tab visible in right dock when `activeTab` set to `'pizarra'`
- [x] Rectangle, circle, line, arrow, textbox shapes can be drawn on canvas
- [x] Shapes can be selected (click) and moved (drag)
- [x] Shapes can be resized via Transformer handles
- [x] Property inspector opens and edits fill, stroke, strokeWidth, opacity
- [x] Theme colors match application chrome (accent-primary, borders, shadows)
- [x] Canvas resizes correctly with right dock panel resizing
- [x] No console errors during shape creation, selection, and property editing
- [x] SSR-safe: canvas does not render on server; hydration works client-side
