...
The activity tracker in `TerminalTTY.jsx` MUST expose `running` | `idle` driven by substantial PTY→client output frames. User input MUST NOT promote `running`. The tracker MUST act as a **fallback** when semantic `agentTuiState` is `unknown` or older than the configured semantic TTL.
...
- GIVEN semantic state is unknown or stale
