...
const PIZARRA_GRID_TEXTURE: boolean =
  typeof process !== 'undefined' && process.env?.NEXT_PUBLIC_PIZARRA_GRID_TEXTURE === '1';
...
**Test strategy reference.** `board-browser-load` Req 1-5 → `PizarraBrowserSurface.test.jsx` (existing WIP-modified) + a new `rightDockState.test.js` for Req 5. `board-browser-pane` Req 1-3 → same `PizarraBrowserSurface.test.jsx` for the address bar / refresh / load indicator, and `src/components/__tests__/TerminalWorkspacesManager.right-dock.test.jsx` (extended existing) for the tab strip 1px inner border.

---

### 3.5 Move 5 — Browser pane chrome + brutalist polish (`board-browser-pane` Req 1-5)

**Current state.** `PizarraBrowserSurface.jsx` renders a drag handle button but NO header chrome (no address bar, no refresh button, no load indicator) — the user sees a window without controls. `PizarraToolPalette.jsx` already uses `btnSecondaryStyle` from the chrome morphology module; hover/active states come from `btnSecondaryStyle` but are not explicit in `PizarraToolPalette` (the test assertion is on the computed `border-color` change). The right-dock tab strip lives in `src/components/TerminalWorkspacesManager.jsx` (lines 3746-3806 — the active-tab buttons at the top of the dock), not in `WorkspaceRightDock.jsx`; the active-tab styling is currently implicit.

> **Archive note (W2 fix)**: this paragraph was corrected at archive time. The original design text named `WorkspaceRightDock.jsx` lines 33-37 as the tab-strip owner, but the actual tab buttons rendered by the right dock live in `src/components/TerminalWorkspacesManager.jsx` (lines 3746-3806). Task 3.7 and the regression test (`src/components/__tests__/TerminalWorkspacesManager.right-dock.test.jsx`) edit the correct file. See `openspec/changes/pizarra-ux-overhaul/tasks.md` Note 1.
...
**Test strategy reference.** `board-browser-pane` Req 1-3 → `PizarraBrowserSurface.test.jsx`. Req 4-5 → same file for header hover; `src/components/__tests__/TerminalWorkspacesManager.right-dock.test.jsx` (extended existing) for the tab strip.
...
- `TerminalWorkspacesManager.jsx` (right-dock tab strip) 1px inner border: no persisted state.
...
| 7     | `src/components/__tests__/TerminalWorkspacesManager.right-dock.test.jsx` (extend existing) | Active tab 1px accent inner border                                                                                                                       | `board-browser-pane` Req 5                                             |
...
### `TerminalWorkspacesManager.right-dock.test.jsx` (2 scenarios, extended existing)
