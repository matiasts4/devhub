# Delta Spec: board-canvas (Pizarra UX Overhaul, Phase 1)
...
> **Stem rationale**: named `board-canvas` per orchestrator instruction. Will be promoted to a base spec at `openspec/specs/board-canvas/spec.md` on archive. Does not duplicate or modify the existing `openspec/specs/pizarra-canvas/spec.md` — that spec covers the higher-level shape model, selection, and dock-tab integration; this delta covers the board surface rendering, first-mount behavior, and the test contract.
