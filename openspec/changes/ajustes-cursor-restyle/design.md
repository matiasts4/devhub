...
`useTerminalSettingsFlag()` (defined above) is module-internal to Ajustes.jsx. The existing terminal preference storage keys (`devhub_terminal_renderer_default_mode`, `devhub_terminal_typography`, etc.) are unchanged — PR-1 only moves the _UI_ surface.
