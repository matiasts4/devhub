...
- Replacing the live surface layer's pre-zoomed-bounds math (the layer keeps its own division by `resolvedZoom` for the position sync; the hook contract is the *delta* contract, not the resolved-position contract).
