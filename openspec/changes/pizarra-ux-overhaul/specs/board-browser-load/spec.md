...
> **Stem rationale**: new capability. Promoted to base spec at `openspec/specs/board-browser-load/spec.md` on archive. Distinct from the existing `openspec/specs/browser-preview-lifecycle/spec.md` (which covers selector activation / cross-origin support classification) and `openspec/specs/browser-preview-responsiveness/spec.md` (which covers bounded polling and diagnostics). This spec owns the *initial mount and runtime mode* contract for pizarra-mounted browser shapes.
