...
DevHub estandariza el camino desktop Linux en **GTK/VTE same-window como renderer nativo por defecto**.

`xterm` sigue siendo el **único fallback estable y soportado** cuando GTK no esté listo, falle el attach nativo, o una sesión necesite recuperación controlada.
...
| TERM-03 | En curso | GTK/VTE ya avanzó más allá del spike inicial, pero todavía faltan verificación focalizada y hardening de compatibilidad. |
| TERM-04 | Cerrado / no-go por ahora | **No se sigue con Ghostty/libghostty** salvo que TERM-03 y la auditoría de compatibilidad demuestren un bloqueo real que GTK/VTE no pueda resolver razonablemente. |
| TERM-05 | Dirección aceptada | Se registran explícitamente las opciones degradadas/descartadas y se fija la estrategia canónica: GTK/VTE + fallback xterm. |
...
1. **GTK/VTE queda como default nativo en Linux/Tauri.**
2. **xterm queda como fallback único y estable.**
3. **Ghostty/libghostty sale del roadmap activo.**
4. **No se aceptan terminales externas/overlay como camino principal.**
5. **No se considera listo TERM-03** hasta auditar OpenCode, Hermes, Swarm y reanudación real.
6. **No se debe arrancar GTK + xterm a la vez** para el mismo panel salvo fallback/recovery explícito.
7. **La configuración del renderer debe vivir en Settings**, no en el header operativo de la terminal.
