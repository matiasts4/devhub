...
export function registerTaskTools(server, deps) {
  ...
  server.tool(
    ...
    async ({ project_id, agent_id }) => {
      try {
        if (DB_DRIVER !== 'supabase') {
          const claimed = getLocalClaimTransaction(deps)({
          ...
          return ok(
            claimed.task
              ? { ...claimed, task: await attachSupervisorToTask(claimed.task, deps) }
              : claimed
          );
        }

        const claimed = await claimNextTaskSupabase(project_id, agent_id, deps);
      ...
      try {
        ...
        if (DB_DRIVER !== 'supabase') {
          ...
          db.prepare(
            ...
            releaseFields.status,
            releaseFields.completed_at,
        ...
        await cleanupExpiredLeases(null, agent_id, deps, nowMs);
        ...
        const activeTask = await getAgentActiveTask(task.project_id, agent_id, deps, nowMs);
        await syncAgentRegistryState(agent_id, deps, {
          currentTaskId: activeTask?.id || null,
          status: activeTask ? 'working' : 'idle',
          lastHeartbeat: timestamp,
        });
