...
export function registerWorkspaceTools(server, deps) {
  ...
  server.tool(
    ...
    async ({ workspace_id, ...input }) => {
      try {
        const existing = await getAgentWorkspaceById(workspace_id, deps);
        if (existing) {
          return ok({
            created: false,
        ...
        if (collisions.length > 0) {
          ...
          const conflict = await insertAgentWorkspace(
            ...
            return insertAgentWorkspace(fallback, deps);
          });
          return ok({
            created: false,
            workspace: conflict,
        ...
        const workspace = await insertAgentWorkspace(payload, deps);
        return ok({
          created: true,
  ...
  server.tool(
    ...
    async ({ workspace_id, ...updates }) => {
      try {
        ...
        const workspace = await updateAgentWorkspaceRow(workspace_id, next, deps);
    ...
    async ({ workspace_id, ...report }) => {
      try {
        ...
        return ok({
          ...
          no_op: false,
          workspace,
          message: 'Observed state registrado.',
        });
      } catch (e) {
        return err(e.message);
      }
  ...
  server.tool(
    ...
    'Crea un header durable en agent_runs sin convertir git/worktree en verbos MCP.',
    {
      ...
      predecessor_run_id: RUN_ID_SCHEMA.optional(),
  ...
  server.tool(
    ...
    async ({ run_id, status, terminal_reason_class, completed_at }) => {
      try {
        const run = await updateAgentRunTerminalRow(
          run_id,
          {
            status,
