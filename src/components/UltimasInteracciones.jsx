...
const interactions = [
  {
    id: 1,
    user: 'Dev Admin',
    action: 'Generó módulo de autenticación',
  ...
  },
  {
    id: 2,
    user: 'NEXUS-7',
    action: 'Revisó 3 componentes UI',
  ...
  },
  {
    id: 3,
    user: 'Pipeline CI/CD',
    action: 'Build exitoso · v0.4.2-beta',
    target: 'Production',
    time: '32 min',
    icon: Rocket,
  ...
  },
  {
    id: 4,
    user: 'NEXUS-3',
    action: 'Sugirió refactoring crítico',
  ...
  },
  {
    id: 5,
    user: 'Sistema',
    action: 'Error detectado y registrado',
    target: 'UserSession.js',
    time: '2h',
    icon: AlertCircle,
    color: 'var(--danger)',
  },
        ...
        <thead>
          <tr className="border-b border-borders-subtle">
            {['Tipo', 'Acción', 'Destino', 'Agente', 'Hace'].map((h) => (
              <th
                key={h}
                className="text-left px-5 py-2 text-[11px] uppercase tracking-[0.12em] text-text-muted font-semibold"
              >
                {h}
              </th>
            ))}
          ...
          {interactions.map((item, i) => {
            ...
            return (
              ...
              >
                ...
                <td className="px-5 py-2.5">
                  <span className="text-xs text-text-muted">hace {item.time}</span>
                </td>
              </tr>
            );
          })}
