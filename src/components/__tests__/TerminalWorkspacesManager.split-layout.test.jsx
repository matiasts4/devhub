...
jest.mock('react-resizable-panels', () => ({
  ...
  PanelResizeHandle: ({ onDragging, onMouseDown, onMouseUp, onPointerDown, onPointerUp, ...props }) => {
    const React = require('react');
...
jest.mock('../TerminalTTY', () => ({
  __esModule: true,
    ...
    React.useEffect(() => {
      ...
      return () => {
        mockWindow.removeEventListener('devhub:terminal-native-vte-event', handleNativeRuntimeEvent);
      };
    }, [id, onActivatePanel]);
      ...
      return React.createElement('div', { 'data-testid': `terminal-${id}` }, [
        ...
        React.createElement('span', { key: 'visible', 'data-testid': `terminal-visible-${id}` }, isVisibleInLayout ? 'visible' : 'hidden'),
        React.createElement('span', { key: 'suspend', 'data-testid': `terminal-suspend-${id}` }, suspendNativeSurface ? 'suspended' : 'live'),
        React.createElement('span', { key: 'renderer', 'data-testid': `terminal-renderer-${id}` }, requestedRendererMode || 'xterm'),
        onActivatePanel
        ? React.createElement(
            'button',
            {
        ...
        : null,
      onResetRendererToXterm
        ? React.createElement('button', { key: 'reset', type: 'button', 'data-testid': `terminal-renderer-reset-${id}`, onClick: onResetRendererToXterm }, 'reset')
        : null,
    ]);
  },
}));
...
jest.mock('@/components/ui/dropdown-menu', () => ({
  DropdownMenu: ({ children, onOpenChange }) => {
    const React = require('react');
    const DropdownMenuContext = global.__DEVHUB_TEST_DROPDOWN_MENU_CONTEXT__
...
describe('TerminalWorkspacesManager split layout', () => {
  ...
  test('renders horizontal splits as side-by-side workspace columns', async () => {
    ...
    const handle = view.container.querySelector('[data-testid="split-column-resize-handle-ws1-c1"]');
  ...
  test('renders vertical splits as stacked panels inside the same column', async () => {
    ...
    const handle = view.container.querySelector('[data-testid="workspace-row-resize-handle-c1-p1"]');
    expect(handle).not.toBeNull();
  ...
  test('keeps two visible split native panels mounted at the same time while focus stays panel-scoped', async () => {
    ...
    expect(view.container.querySelector('[data-testid="terminal-visible-p1"]')?.textContent).toBe('visible');
    expect(view.container.querySelector('[data-testid="terminal-visible-p2"]')?.textContent).toBe('visible');
    expect(view.container.querySelector('[data-testid="terminal-active-p1"]')?.textContent).toBe('inactive');
    expect(view.container.querySelector('[data-testid="terminal-active-p2"]')?.textContent).toBe('active');
  });
  ...
  test('renders minimal semantic metadata derived from the initial command', async () => {
      ...
      workspaces: [
        {
          ...
          name: 'Workspace 1',
          columns: [
            { id: 'c1', panels: [{ id: 'p1', cwd: '/workspace/devhub', initialCommand: 'opencode --agent sdd-explore' }] },
          ],
        },
      ],
      activeWsId: 'ws1',
    ...
    expect(view.container.querySelector('[data-testid="panel-semantic-primary-p1"]')?.textContent).toBe('OpenCode');
    expect(view.container.querySelector('[data-testid="panel-semantic-secondary-p1"]')?.textContent).toBe('sdd-explore');
  });
      ...
      workspaces: [
        {
          ...
          name: 'Workspace 1',
          columns: [
            { id: 'c1', panels: [{ id: 'p1', cwd: '/workspace/devhub', initialCommand: 'opencode --agent sdd-apply' }] },
          ],
        },
      ],
      activeWsId: 'ws1',
  ...
  test('wires reset and per-panel close intent while keeping the visible sibling mounted', async () => {
    ...
    view.container.querySelector('[data-testid="terminal-renderer-reset-p1"]')?.dispatchEvent(
      new window.MouseEvent('click', { bubbles: true })
    );
    await Promise.resolve();

    expect(view.container.querySelector('[data-testid="terminal-renderer-p1"]')?.textContent).toBe('xterm');
    expect(view.container.querySelector('[data-testid="terminal-visible-p2"]')?.textContent).toBe('visible');

    view.container.querySelector('[data-testid="panel-slot-p1"] button[aria-label="Cerrar terminal"]')?.dispatchEvent(
      new window.MouseEvent('click', { bubbles: true })
    );
    await Promise.resolve();
  ...
  test('renders floating per-panel chrome outside the terminal render body so native content does not cover controls', async () => {
    ...
    const panelSemanticHeader = view.container.querySelector('[data-testid="panel-semantic-header-p1"]');
    ...
    expect(panelChromeOverlay?.querySelector('[data-testid="panel-split-right-p1"]')).toBe(splitRightButton);
    expect(panelChromeOverlay?.querySelector('[data-testid="panel-split-down-p1"]')).toBe(splitDownButton);
  ...
  test('keeps per-panel split controls working from the floating overlay chrome', async () => {
    ...
    view.container.querySelector('[data-testid="panel-split-right-p1"]')?.dispatchEvent(
      new window.MouseEvent('click', { bubbles: true })
    );
    await Promise.resolve();

    const columns = Array.from(view.container.querySelectorAll('[data-testid^="workspace-column-"]'));
  ...
  test('uses the clicked terminal as the split source for global split controls', async () => {
    ...
    view.container.querySelector('[data-testid="terminal-activate-p1"]')?.dispatchEvent(
      new window.MouseEvent('mousedown', { bubbles: true })
    );
    await Promise.resolve();

    expect(view.container.querySelector('[data-testid="terminal-active-p1"]')?.textContent).toBe('active');
    expect(view.container.querySelector('[data-testid="terminal-active-p2"]')?.textContent).toBe('inactive');

    view.container.querySelector('[data-testid="panel-subtabs-split-right"]')?.dispatchEvent(
      new window.MouseEvent('click', { bubbles: true })
    );
    await Promise.resolve();
  ...
  test('suspends native surfaces while Grid dropdown is open and restores them when it closes', async () => {
    ...
    expect(view.container.querySelector('[data-testid="terminal-suspend-p2"]')?.textContent).toBe('live');

    trigger?.dispatchEvent(new window.MouseEvent('click', { bubbles: true }));
    await Promise.resolve();

    expect(view.container.querySelector('[data-testid="terminal-suspend-p1"]')?.textContent).toBe('suspended');
    expect(view.container.querySelector('[data-testid="terminal-suspend-p2"]')?.textContent).toBe('suspended');
