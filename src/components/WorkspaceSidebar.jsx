...
import {
  ...
  ChevronRight, Settings, History, ArrowLeft, Code2, Plug2, FolderOpen, Terminal, Brain, Cpu
} from "lucide-react";
...
const DEFAULT_NAV = ["dashboard", "tareas", "editor", "roadmap", "historial", "agentes", "planning", "swarm"];

export default function WorkspaceSidebar({ project, collapsed, onToggle, onToggleTerminal, isTerminalOpen }) {
  ...
  const visibleNavKeys = project?.features?.length > 0 
  ...
  return (
    ...
    >
        ...
        <div className="px-3 py-3 border-b" style={{ borderBottomColor: "var(--border-subtle)" }}>
          <div className="flex items-center gap-2.5">
            ...
            <div className="min-w-0">
              <p className="font-mono text-xs font-semibold truncate leading-tight" style={{ color: "var(--text-primary)" }}>{project?.name}</p>
              <p
      ...
      {collapsed && (
        <div className="flex justify-center py-3 border-b" style={{ borderBottomColor: "var(--border-subtle)" }}>
          <div
      ...
      <nav className="flex-1 overflow-y-auto py-2 px-2 space-y-0.5">
        {!collapsed && (
          <p className="px-2.5 pt-1 pb-1.5 text-[9px] uppercase tracking-[0.15em] font-semibold" style={{ color: "var(--text-muted)" }}>
            Proyecto
          </p>
        )}
        {visibleNavKeys.map((key) => {
          ...
          return (
            <Link
              ...
              style={isActive(key) ? { background: "var(--surface-elevated)", boxShadow: "var(--shadow-soft)" } : undefined}
            >
        ...
        {!collapsed && (
          <p className="px-2.5 pt-0.5 pb-1.5 text-[9px] uppercase tracking-[0.15em] font-semibold" style={{ color: "var(--text-muted)" }}>
            Herramientas
          </p>
        )}
        ...
        <Link
          ...
          style={isActive('terminales') ? { background: "var(--surface-elevated)", boxShadow: "var(--shadow-soft)" } : undefined}
        >
      ...
      {!collapsed && project?.progress !== undefined && (
        <div className="px-3 pb-3 border-t pt-3" style={{ borderTopColor: "var(--border-subtle)" }}>
          <div className="flex justify-between mb-1">
            <span className="text-[9px] uppercase tracking-wider font-semibold" style={{ color: "var(--text-muted)" }}>Progreso</span>
            <span className="text-[9px] font-mono" style={{ color: accentColor }}>{project.progress}%</span>
          </div>
      ...
      <button
        ...
        style={{ borderTopColor: "var(--border-subtle)", color: "var(--text-muted)", background: "var(--surface-card)" }}
      >
        {collapsed
