    ...
    <div className="flex flex-col h-full bg-surface-base border-t md:border-l border-borders-subtle mt-4 md:mt-0 p-4">
      ...
      >
        {loading ? (
          ...
          <p className="text-xs text-text-muted text-center italic">No hay comentarios aún</p>
        ) : null}

        {comments.map((c) => (
          <div
            key={c.id}
            className={`flex gap-3 text-sm p-3 rounded-lg border ${c.author_type === 'agent' ? 'border-[#F778BA]/30 bg-[#F778BA]/5' : 'border-borders-subtle bg-surface-card'}`}
          >
