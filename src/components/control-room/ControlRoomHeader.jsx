    ...
    >
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div className="space-y-2">
          ...
          <h1 className="text-2xl font-semibold">{projectName || 'Workspace Control Room'}</h1>
        ...
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          <MetricCard label="Agents" value={`${header.active}/${header.max} agents active`} />
          <MetricCard label="Queue" value={`${header.queue_depth} queued`} />
          <MetricCard label="Authority" value={formatToken(header.authority)} />
          <MetricCard label="Freshness" value={formatToken(header.freshness)} />
        </div>
      </div>

      <div className="mt-4 rounded-xl border px-3 py-3 text-sm" style={panelShellStyle()}>
        <span style={metaTextStyle()}>Evidence: </span>
