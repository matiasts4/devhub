...
export default function TareasActivas() {
  return (
    <div data-testid="tareas-activas" className="bg-surface-card border border-borders-subtle rounded-xl overflow-hidden">
      <div className="flex items-center justify-between px-5 py-3.5 border-b border-borders-subtle">
        <div className="flex items-center gap-2.5">
          ...
          <h3 className="font-mono text-sm font-semibold text-text-primary">Tareas Activas de Agentes</h3>
        </div>
          ...
          return (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p className="text-xs font-medium text-text-primary truncate">{task.task}</p>
                <div className="flex items-center gap-2 mt-0.5">
                  <span className="text-[10px] text-text-muted">{task.agent}</span>
                  <span className="text-[10px] text-[#30363D]">·</span>
                  <span className="text-[10px] text-text-muted">{task.module}</span>
                </div>
                {task.status === "running" && (
