...
describe('applyZedOpenUrlDockUpdate (T-WSR-zed-003)', () => {
  ...
  test('focus opens browser from zed-only dock state', () => {
    const dockState = makeFixture({ activeTab: 'zed', zedVisible: true });
    const next = applyZedOpenUrlDockUpdate(dockState, {
      url: 'https://github.com',
      focus: true,
    });
  ...
  test('with focus, exits pizarra and opens browser + zed', () => {
    const dockState = makeFixture({ maximized: true, maximizedView: 'pizarra' });
    const next = applyZedOpenUrlDockUpdate(dockState, {
      url: 'https://github.com',
      focus: true,
