...
const EMBEDDED_RUNTIME_STATUS_RE = /^Activo: (WebView2 \(Chromium\)|WKWebView|motor embebido)$/;

function expectEmbeddedRuntimeStatus(container) {
  const status =
    container.querySelector('[data-testid="browser-runtime-status"]')?.textContent || '';
  expect(status).toMatch(EMBEDDED_RUNTIME_STATUS_RE);
}
...
describe('WorkspaceBridgePane', () => {
  ...
  test('shows native runtime shell when native browser runtime is active', async () => {
    ...
    expectEmbeddedRuntimeStatus(view.container);
