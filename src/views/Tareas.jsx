...
import {
  ListTodo, Plus, X, ChevronDown, Loader2, Flag, Calendar,
  Trash2, Copy, Check, Filter, Bot, LayoutDashboard, Zap,
  ShieldAlert, Search, Milestone, ChevronRight
} from "lucide-react";
...
const selectStyles = {
  ...
  menu: (base) => ({ ...base, background: '#161b26', border: '1px solid #30363d', borderRadius: '10px', overflow: 'hidden', boxShadow: '0 8px 24px rgba(0,0,0,0.4)' }),
  option: (base, state) => ({ ...base, background: state.isFocused ? '#1c2333' : '#161b26', color: '#f0f6fc', fontSize: '12px' }),
  multiValue: (base) => ({ ...base, background: '#1c2333', borderRadius: '6px' }),
  multiValueLabel: (base) => ({ ...base, color: '#f0f6fc', fontSize: '11px' }),
  multiValueRemove: (base) => ({ ...base, color: '#8b949e', '&:hover': { background: '#F778BA20', color: '#F778BA' } }),
...
function StyledSelect({ label, value, onChange, options, placeholder }) {
  return (
    <div>
      {label && <label className="block text-[10px] text-text-muted font-semibold uppercase tracking-wider mb-1.5">{label}</label>}
...
function TaskModal({ projectId, userId, initialStatus, existingTask, allTasks, milestones, dependencies, onClose, onSaved }) {
    ...
    if (targetTaskId) {
      ...
      if (selectedDeps.length > 0) {
        await supabase.from("task_dependencies").insert(
          selectedDeps.map(d => ({ task_id: targetTaskId, depends_on: d.value, tipo: "blocks" }))
        );
      }
    }
  ...
  return (
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={onClose}>
      <div className="bg-surface-card border border-borders-strong rounded-2xl w-full max-w-lg shadow-2xl flex flex-col max-h-[92vh]" onClick={e => e.stopPropagation()}>
        ...
        <div className="overflow-y-auto p-5 flex-1">
          <form id="task-form" className="space-y-4" onSubmit={handleSubmit}>
            ...
            {/* Title */}
            <div>
              <label className="block text-[10px] text-text-muted font-semibold uppercase tracking-wider mb-1.5">Título *</label>
              <input
                required
                value={form.title}
            ...
            {/* Description */}
            <div>
              <label className="block text-[10px] text-text-muted font-semibold uppercase tracking-wider mb-1.5">Descripción</label>
              <textarea
                rows={3}
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                <label className="block text-[10px] text-text-muted font-semibold uppercase tracking-wider mb-1.5">Prioridad</label>
                <div className="grid grid-cols-2 gap-1.5">
                  {Object.entries(PRIORITY).map(([k, v]) => (
                    <button
                      ...
                      style={
                        form.priority === k
                          ? { background: `${v.color}18`, borderColor: `${v.color}40`, color: v.color }
              ...
              <div>
                ...
                <div className="pt-3">
                  <input
                    ...
                    onChange={e => setForm(p => ({ ...p, business_value: parseInt(e.target.value) }))}
            ...
            {/* Due date */}
            <div>
              <label className="block text-[10px] text-text-muted font-semibold uppercase tracking-wider mb-1.5">Fecha Límite</label>
              <DatePicker
                value={form.due_date}
            ...
            <div className="pt-1 border-t border-borders-subtle">
              <label className="block text-[10px] text-text-muted font-semibold uppercase tracking-wider mb-1.5">Depende de (Bloqueada por)</label>
              <Select
                isMulti
                options={taskOptions}
                value={selectedDeps}
            ...
            {blocksTasks.length > 0 && (
              <div className="bg-surface-elevated p-3 rounded-lg border border-borders-subtle">
                <p className="text-[10px] text-text-muted mb-1.5 font-semibold uppercase tracking-wider">Bloquea a ({blocksTasks.length}):</p>
    ...
    <div className="space-y-3">
      <div className="bg-surface-elevated border border-borders-subtle rounded-xl p-4 flex items-start gap-4">
        ...
        <div>
          ...
          <p className="text-xs text-text-muted leading-relaxed">
            Tareas ordenadas por urgencia, valor de negocio y desbloqueadores. Solo incluye tareas libres de bloqueos.
          </p>
        </div>
      </div>
      ...
      {queue.length === 0 ? (
        ...
        <div className="space-y-2.5 mt-2">
          {queue.map((task, i) => (
            <div key={task.id} className="bg-surface-card border border-borders-subtle rounded-xl p-4 flex items-center justify-between group hover:border-[#58A6FF]/30 transition-all">
              <div className="flex-1 flex items-center gap-4">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-3 mt-1 opacity-80">
                    <span className="text-[10px] flex items-center gap-1" style={{ color: task.priorityObj.color }}>
                      <Flag className="w-3 h-3" /> {task.priorityObj.label}
                    </span>
                    {task.m_title && (
                      <span className="text-[10px] text-text-muted bg-surface-elevated px-1.5 py-0.5 rounded-md border border-borders-subtle">{task.m_title}</span>
                    )}
              ...
              >
                {copiedTask === task.id ? <Check className="w-3.5 h-3.5" /> : <Zap className="w-3.5 h-3.5" />}
                {copiedTask === task.id ? "¡Copiado!" : "Ejecutar con Worker"}
              </button>
            </div>
          ))}
        </div>
    ...
    <div className="min-h-screen bg-surface-app flex flex-col">
      ...
      <div className="sticky top-0 z-10 bg-surface-app/95 backdrop-blur-sm border-b border-borders-subtle px-6 py-3 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={() => { setEditingTask(null); setInitialStatus("pending"); setModalOpen(true); }}
          ...
          <div className="bg-surface-card border border-borders-subtle rounded-xl p-3 flex flex-wrap items-center gap-2.5">
            ...
            <div className="flex items-center gap-1.5 text-text-muted pr-1 border-r border-borders-subtle mr-1">
              ...
              {activeFiltersCount > 0 && (
                <span className="text-[9px] bg-[#58A6FF] text-white rounded-full w-4 h-4 flex items-center justify-center font-bold">{activeFiltersCount}</span>
              )}
            </div>
            ...
            {activeFiltersCount > 0 && (
              <button
                onClick={() => { setFSearch(""); setFMilestone(""); setFUnlocked(false); setFMyTasks(false); }}
            ...
            {COLUMNS.map(col => {
              ...
              return (
                <div key={col.id} className="bg-surface-card border border-borders-subtle rounded-xl overflow-hidden flex flex-col max-h-[80vh]">
                  {/* Column header */}
                  <div className="flex items-center justify-between px-4 py-3 border-b border-borders-subtle" style={{ borderTop: `2px solid ${col.color}` }}>
                    <div className="flex items-center gap-2">
                  ...
                  <div className="p-2.5 flex-1 overflow-y-auto space-y-2 custom-scrollbar">
                    {colTasks.map((task) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <p className="text-xs text-text-primary font-medium leading-snug mb-2.5 pr-5">{task.title}</p>
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-2">
                              <span className="text-[9px] font-semibold flex items-center gap-0.5" style={{ color: prio.color }}>
                                <Flag className="w-2.5 h-2.5" />
                              </span>
                              {task.business_value && (
                                <span className="text-[9px] text-[#58A6FF] font-mono" title="Valor Negocio">V:{task.business_value}</span>
                              )}
                              {task.due_date && (
                    ...
                    <button
                      onClick={() => { setEditingTask(null); setInitialStatus(col.id); setModalOpen(true); }}
