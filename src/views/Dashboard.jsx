  ...
  const handleStaleTasks = async (tasks) => {
      const resetTasks = async () => {
          for (const t of tasks) {
              await supabase.from('tasks').update({ status: 'pending', stale_alert: false, priority: 'medium' }).eq('id', t.id);
          }
      ...
      const escalateTasks = async () => {
          for (const t of tasks) {
              await supabase.from('tasks').update({ priority: 'critical', stale_alert: false }).eq('id', t.id);
          }
      ...
      toast(
          <div className="flex flex-col gap-2">
              <div className="flex items-center gap-2 text-warning"><AlertCircle className="w-5 h-5"/> <h4 className="font-semibold text-sm">Tareas Estancadas Detectadas</h4></div>
              <p className="text-xs text-text-muted">Hay {tasks.length} tareas en progreso por más de 48h en el proyecto.</p>
              <div className="flex gap-2 mt-2">
                  <button onClick={() => {toast.dismiss(); resetTasks();}} className="px-2 py-1 text-xs bg-surface-elevated hover:bg-surface-active rounded border border-borders-subtle">Resetear a Pendiente</button>
                  <button onClick={() => {toast.dismiss(); escalateTasks();}} className="px-2 py-1 text-xs bg-red-500/10 hover:bg-red-500/20 text-red-400 rounded border border-red-500/30">Escalar Prioridad</button>
              </div>
          </div>,
          { duration: 15000, unstyled: true, classNames: { toast: "bg-surface-card border border-borders-strong p-4 rounded-xl shadow-xl w-full" } }
      );
  };
  ...
  useEffect(() => {
     if (project?.id) {
         ...
         const checkStale = async () => {
             ...
             const { data: staleData } = await supabase.from("tasks")
                 .select("id")
             ...
             if (staleData && staleData.length > 0) {
                 await supabase.from("tasks").update({ stale_alert: true }).in("id", staleData.map(t => t.id));
             }

             // Actual notification query
             const { data: alerts } = await supabase.from("tasks")
                 .select("id, title")
                 .eq("project_id", project.id)
                 .eq("stale_alert", true);
    ...
    <div className="min-h-screen bg-surface-app dot-grid">
      ...
      <div className="px-6 py-5 space-y-5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {metricCards.map((card, index) => <MetricCard key={card.id} {...card} index={index} />)}
        </div>
