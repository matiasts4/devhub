...
describe('App.js — Ajustes route (settings/* removed)', () => {
  ...
  test('does NOT import the removed settings page components', () => {
    expect(app).not.toMatch(/import\s+AppearancePage\s+from\s+['"]\.\/app\/settings\/appearance\/page['"]/);
    expect(app).not.toMatch(/import\s+AccountPage\s+from\s+['"]\.\/app\/settings\/account\/page['"]/);
    expect(app).not.toMatch(
  ...
  test('mounts Ajustes directly at /project/:projectId/ajustes', () => {
    ...
    expect(app).toMatch(
      /<Route\s+path=["']ajustes["']\s+element=\{<Ajustes\s*\/\>\}\s*\/>/
    );
