...
export const POST = withAuth(async function POST(request) {
  try {
    ...
    const { mission_id, agent_id, workspace_id, run_id, state, cwd, status_summary } = body;
    ...
    // Normalize separators: Windows agents report `D:\...\ .devhub\worktrees\...`.
    if (cwd && !String(cwd).replace(/\\/g, '/').includes('.devhub/worktrees')) {
