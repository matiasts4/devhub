...
describe('DG timeline row factory', () => {
  describe('emitRow — action/status matrix', () => {
    test('mission-request + pending produces correct row shape', async () => {
      ...
      const mockFetch = makeMockFetch(fullRow({ action: 'mission-request', status: 'pending', fallback: 'Operator puede aprobar o cancelar.' }));
      const row = await emitRow('mission-request', 'pending', {
        missionId: 'mission-1',
        initiator: 'operator',
        target: 'swarm-director',
        authority: 'operator-initiated',
    ...
    test('status-poll + in-progress produces correct row shape', async () => {
      const { emitRow } = require('../timeline');
      const mockFetch = makeMockFetch(fullRow({ action: 'status-poll', status: 'in-progress', initiator: 'director-general' }));
      const row = await emitRow('status-poll', 'in-progress', {
        missionId: 'mission-1',
    ...
    test('approval-required + awaiting-approval produces correct row shape', async () => {
      const { emitRow } = require('../timeline');
      const mockFetch = makeMockFetch(fullRow({ action: 'approval-required', status: 'awaiting-approval', initiator: 'swarm-director', target: 'operator', authority: 'operator' }));
      const row = await emitRow('approval-required', 'awaiting-approval', {
        missionId: 'mission-1',
    ...
    test('mission-result + completed produces correct row shape', async () => {
      const { emitRow } = require('../timeline');
      const mockFetch = makeMockFetch(fullRow({ action: 'mission-result', status: 'completed', initiator: 'swarm-director', target: 'operator', authority: 'director' }));
      const row = await emitRow('mission-result', 'completed', {
        missionId: 'mission-1',
        initiator: 'swarm-director',
        target: 'operator',
        authority: 'director',
    ...
    test('mission-result + failed produces row with fallback', async () => {
      const { emitRow } = require('../timeline');
      const mockFetch = makeMockFetch(fullRow({ action: 'mission-result', status: 'failed', authority: 'director-escalated', fallback: 'Contactá al supervisor directo.' }));
      const row = await emitRow('mission-result', 'failed', {
        missionId: 'mission-1',
        initiator: 'swarm-director',
        target: 'operator',
    ...
    test('status-poll + waiting produces correct row shape', async () => {
      ...
      const row = await emitRow('status-poll', 'waiting', {
        missionId: 'mission-1',
        initiator: 'director-general',
        target: 'swarm-director',
        authority: 'operator-initiated',
    ...
    test('status-poll + rejected produces correct row shape', async () => {
      ...
      const row = await emitRow('status-poll', 'rejected', {
        missionId: 'mission-1',
        initiator: 'director-general',
        target: 'swarm-director',
        authority: 'operator-initiated',
  ...
  describe('emitRow — authority validation (DG MUST NOT)', () => {
    test('throws when initiator=operator and authority=director', async () => {
      ...
      await expect(
        emitRow('mission-request', 'pending', {
          missionId: 'mission-1',
          initiator: 'operator',
          target: 'swarm-director',
          authority: 'director',
    ...
    test('throws when initiator=director-general and authority=director-escalated', async () => {
      ...
      await expect(
        emitRow('status-poll', 'in-progress', {
          missionId: 'mission-1',
          initiator: 'director-general',
          target: 'swarm-director',
          authority: 'director-escalated',
    ...
    test('throws when initiator=swarm-director and authority=operator-initiated', async () => {
      ...
      await expect(
        emitRow('mission-result', 'completed', {
          missionId: 'mission-1',
          initiator: 'swarm-director',
          target: 'operator',
          authority: 'operator-initiated',
  ...
  describe('emitRow — POST behavior', () => {
    test('POSTs to the correct timeline endpoint', async () => {
      ...
      await emitRow('mission-request', 'pending', {
