...
function buildTelegramIntentIdempotencyKey({ update_id, message_id, actor_id, action, target_ref = {} }) {
...
function getTelegramActorMappingByActorId(dbOrActorId, maybeActorId) {
  ...
  return normalizeTelegramActorRow(
    db.prepare('SELECT * FROM telegram_actor_mappings WHERE actor_id = ? LIMIT 1').get(actorId) || null
  );
}
...
function upsertTelegramActorMapping(dbOrInput, maybeInput) {
  ...
  const row = {
    ...
    telegram_chat_id: input.telegram_chat_id ? String(input.telegram_chat_id) : existing?.telegram_chat_id || null,
...
function getTelegramIntentByIdempotencyKey(dbOrKey, maybeKey) {
  ...
  return normalizeTelegramIntentRow(
    db.prepare('SELECT * FROM telegram_intent_envelopes WHERE idempotency_key = ? LIMIT 1').get(key) ||
      null
  );
}
...
function recordTelegramIntentEnvelope(dbOrInput, maybeInput) {
  ...
  if (!actor.allowlisted) throw new Error(`actor_id no allowlisted para telegram intents: ${input.actor_id}`);
...
function buildTelegramDeliveryKey({ intent_id = null, task_id = null, workspace_id = null, run_id = null, telegram_chat_id }) {
  return ['delivery', intent_id || '-', task_id || '-', workspace_id || '-', run_id || '-', telegram_chat_id || '-'].join(':');
}

function getLatestTelegramDeliveryReceipt(db, { task_id = null, workspace_id = null, run_id = null } = {}) {
  const clauses = [];
  const params = [];
  if (run_id) {
    clauses.push('run_id = ?');
...
function upsertTelegramDeliveryReceipt(dbOrInput, maybeInput) {
  ...
  const existing = normalizeTelegramDeliveryRow(
    db.prepare('SELECT * FROM telegram_delivery_receipts WHERE delivery_key = ? LIMIT 1').get(deliveryKey) || null
  );
...
function upsertTelegramSubscription(dbOrInput, maybeInput) {
  ...
  const existing = normalizeTelegramSubscriptionRow(
    db.prepare('SELECT * FROM telegram_subscriptions WHERE subscription_key = ? LIMIT 1').get(subscriptionKey) || null
  );
  const timestamp = input.updated_at || new Date().toISOString();
  const row = {
...
function getLatestTelegramChannelSnapshot(dbOrFilters, maybeFilters) {
  ...
  const artifactCount = run?.run_id
    ? Number(db.prepare('SELECT count(*) as cnt FROM agent_artifacts WHERE run_id = ?').get(run.run_id)?.cnt || 0)
    : 0;
