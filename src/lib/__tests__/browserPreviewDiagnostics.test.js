...
describe('browserPreviewDiagnostics', () => {
  test('normalizes browser preview diagnostics into stable reason categories', () => {
    expect(
      buildBrowserPreviewDiagnostic('selector-state-transition', {
    ...
    expect(categorizePreviewReason('cross-origin-no-instrumentation')).toBe(
  ...
  test('normalizes preview proxy diagnostics into stable rewrite categories', () => {
    expect(
      buildPreviewProxyDiagnostic('proxy-navigation-escape-risk', {
  ...
  test('builds deterministic dedupe keys from repeated preview diagnostics', () => {
    const first = buildPreviewDiagnosticDedupeKey(
      buildBrowserPreviewDiagnostic('selector-state-transition', {
        reason: 'proxy-escaped',
        supportMode: 'unsupported',
    ...
    const second = buildPreviewDiagnosticDedupeKey(
      buildBrowserPreviewDiagnostic('selector-state-transition', {
        reason: 'proxy-escaped',
        supportMode: 'unsupported',
        browserUrl: { href: 'http://localhost:3200/app' },
    ...
    const third = buildPreviewDiagnosticDedupeKey(
      buildBrowserPreviewDiagnostic('selector-state-transition', {
