...
export async function fetchGrokQuota() {
  ...
  try {
    ...
    // Query Grok billing / credit probe
    try {
      ...
      // Fallback to local credential status check
    }

    // Default optimistic state when auth exists but API is unreachable
...
/**
 * Detect Grok Credentials from ~/.grok/auth.json
 */
function detectGrokCredentials() {
  ...
  try {
    ...
    // Search for OIDC SuperGrok key or fallback session key
