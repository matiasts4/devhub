...
const DESTRUCTIVE_TOOLS = new Set(['close_terminal', 'close_all_terminals', 'delete_project']);
...
export function checkCommandSafety(command) {
  ...
  if (/\brm\s+.*-rf\b/.test(lower))
    return { safe: false, reason: 'recursive deletion is not allowed' };
  if (/curl\s+.*\|\s*(ba)?sh\b/i.test(command))
    return { safe: false, reason: 'curl piped to shell is not allowed' };
