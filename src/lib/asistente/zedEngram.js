...
export async function searchZedMemories({ query, project, scope = 'project', limit = 5 } = {}) {
  ...
  try {
    ...
    memories = result.content ? result.content.split('\n').filter((line) => line.trim()) : [];
  }
...
export function extractZedMemoryEntities(message) {
  ...
  const toolMatches = text.match(
    /\b(open_terminal|execute_in_terminal|open_url|list_terminals|review_terminal_output|create_task|create_milestone)\b/g
  );
  if (toolMatches) {
